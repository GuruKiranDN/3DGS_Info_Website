  ...
  return (
    <main>
      ...
      <h2>3D-Gaussian Splatting</h2>
        ...
        <p>
          It starts with 2D images from all the angles. These photos then run through the Structure from Motion (SfM) for sparse point clouds. Then these sparse point clouds are populated with fuzzy 3D ellipsoids, which hold properties like position, size, rotation, color, and opacity. "Splatting" is the rendering process where 3D Gaussians are projected onto your 2D screen. First, all Gaussians are sorted from front to back based on their distance from the camera. Then each 3D Gaussian is projected onto the screen, creating a 2D elliptical blob. These blobs are blended together using alpha compositing (like Photoshop layers), where closer Gaussians appear on top of farther ones. The result is a photorealistic image.
        </p>
